...
//=============================================================================================================================
	while (iter < 300)
	{
		...
		res = ring_tree_FD(iter, fd, comm, &sp);
		...
		if (iter % 10 == 0 && res == FD_SUCCESS && sp.Lagging_procs.num > 0) {
			retrieve_procs(iter, comm, fd.T_retrieve, &sp);
...
//=============================================================================================================================
	...
	//毫秒为单位
	//double start_time = (pro_start.tv_sec * 1000.0 + pro_start.tv_usec / 1000.0);
	//double end_time = (pro_end.tv_sec * 1000.0 + pro_end.tv_usec / 1000.0);
	//double max_end, min_start;
	////reduce max(end_time) and min(start_time)
	////printf("rank %d, start time is %lf, end time is %lf\n", my_rank, start_time, end_time);
	//MPI_Reduce(&start_time, &min_start, 1, MPI_DOUBLE, MPI_MIN, 0, comm);
	//MPI_Reduce(&end_time, &max_end, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
